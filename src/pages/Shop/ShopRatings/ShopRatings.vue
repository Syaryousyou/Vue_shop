    ...
    <div class="ratings-content">
      ...
      <div class="ratingselect">
        <div class="rating-type border-1px">
          <span class="block positive" :class="{active: selectType === 2}" @click="selsectedType(2)">
            全部<span class="count">{{shopRatings.length}}</span>
          </span>
          <span class="block positive" :class="{active: selectType === 0}" @click="selsectedType(0)">
            满意<span class="count">{{positiveSize}}</span>
          </span>
          <span class="block negative" :class="{active: selectType === 1}" @click="selsectedType(1)">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      filterRatings () {
        const {isShowText, selectType} = this
        return this.shopRatings.filter((rate, index) => {
          /*
            isShowText:
              true: 看rate.text属性是否有值
              false: 全部的内容
              !isShowText || rate.text
            selectType:
              2: 全部  selectType === 2
              1或0： selectType ？ rate.rateType = 1 : rate.rateType = 0
              selectType === 2 || selectType ===rate.rateType
          */
          return (!isShowText || rate.text) && (selectType === 2 || selectType === rate.rateType)
        })
      }
    },
    methods: {
